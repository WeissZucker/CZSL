...
class CGE(nn.Module):
    def __init__(self, hparam, dset, train_only=True, static_inp=True, graph_path=None):
        ...
        if self.train_only:
          train_idx = []
          for current in self.dset.train_pairs:
            train_idx.append(self.dset.all_pair2idx[current]+self.nattrs+self.nobjs)
          self.train_idx = torch.LongTensor(train_idx).to(dev)

        graph = torch.load(graph_path)
          
        self.embeddings = graph['embeddings'].to(dev)
        if not static_inp:
          self.embeddings = nn.Parameter(self.embeddings)
        
        adj = graph['adj']
        
        self.hparam.add('hidden_layers', 'd4096,d')
        self.hparam.add('shared_emb_dim', 512)
        self.gcn = GCN(adj, self.embeddings.shape[1], self.hparam.shared_emb_dim, self.hparam.hidden_layers)
